...
\begin{table}[!t]
    ...
    \begin{tabular}{p{0.18\linewidth} p{0.34\linewidth} p{0.42\linewidth}}
        ...
        \texttt{fast} \texttt{Threshold} & Threshold $t$ in the FAST segment test & Fewer corners (stricter detection); often fewer false positives, but weak points can be lost \\
        ...
        \texttt{edge} \texttt{Threshold} & Border margin where features are suppressed & Avoids keypoints too close to the border; can remove useful boundary points \\
...
\begin{table}[!t]
    ...
    \begingroup
    \renewcommand{\arraystretch}{1.15}
    \begin{tabular}{p{0.16\linewidth} p{0.34\linewidth} p{0.37\linewidth}}
        \hline
        ...
        \texttt{nOctave} \texttt{Layers} & Sub-levels per octave & Finer scale sampling, higher computational cost \\
        ...
        \texttt{diffu} \texttt{-sivity} & Diffusion type (edge-preserving) & Changes the smoothing/edge-preservation tradeoff, affecting keypoint distribution and stability \\
        \hline
    \end{tabular}
    \endgroup
\end{table}
