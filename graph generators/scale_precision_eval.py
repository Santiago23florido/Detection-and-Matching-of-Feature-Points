...
def main() -> None:
    ...
    for s in scales:
        M = cv2.getRotationMatrix2D(center, 0.0, float(s))
        img2 = cv2.warpAffine(
            img1,
            M,
            (w, h),
    ...
    out_path = output_dir / "scale_precision_orb_kaze.png"
    ...
    plt.plot(scales, orb_prec, color="tab:blue", label=f"ORB (avg pairs={orb_pairs_mean})")
    plt.plot(scales, kaze_prec, color="tab:orange", label=f"KAZE (avg pairs={kaze_pairs_mean})")
    plt.xlabel("Scale")
    plt.ylabel("Precision")
    plt.title("Precision vs Scale")
